...
class VzenseTofCam():
    ...
    def __init__(self):
        if platform.system() == 'Linux':
            ...
            libpath = os.path.join(api_dir, 'Lib', 'libvzense_api.so')  
            print("Lib directory is:", libpath)
            self.ps_cam_lib = cdll.LoadLibrary(libpath)
        elif platform.system() == 'Windows':
            # Windows系统
            # 添加Lib目录到环境变量PATH
            api_dir = os.path.dirname(os.path.abspath(__file__))
            libpath = os.path.join(api_dir, 'Lib', 'vzense_api.dll')
            print("Lib directory is:", libpath)
            self.ps_cam_lib = cdll.LoadLibrary(libpath) 
        else:
